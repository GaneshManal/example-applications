"""
Name:       src.py
Purpose:    Test script to push data into kafka for consumption by the example network interface data processing application using flink streaming.
...
def run(brokers, record_count, host_count, int_per_host):
    ...
    for x_host in range(1, host_count+1):
        ...
        for x_int in range(int_per_host):
            ...
            current_int['upload-speed-bytes'], current_int['download-speed-bytes'] = 100, 150
    ...
    for item in range(0, record_count):
        ...
        writer.write({"source": "hndu-src", "timestamp": time_sec, "rawdata": json.dumps(data)}, encoder)
...
if __name__ == '__main__':
    ...
    broker_list = None
    if len(sys.argv) != 5:
        print "Usage: python src.py <kafka_broker:port> number_of_records total_hosts interfaces_per_host"
